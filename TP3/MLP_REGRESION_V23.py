...
def ejecutar_adelante(x, pesos,activacion):
...
def calcular_loss(x, t, pesos,activacion):
    ejecucion = ejecutar_adelante(x, pesos,activacion)
    y_pred = ejecucion["y"]
...
def train(x, t, xtest, ttest, pesos, learning_rate, epochs, activacion):  
    m = x.shape[0]  
    ...
    for i in range(epochs):
        loss, resultados = calcular_loss(x, t, pesos,activacion)
        lista_loss.append(loss)
        lossval, _ = calcular_loss(xval, tval, pesos,activacion)
        ...
        mse_prueba ,_= calcular_loss(xtest, ttest, pesos,activacion)
    ...
    return pesos, lista_precision_prueba[-1]


def iniciar( n_entrada, n_capa_2, n_capa_3, learning_rate, epochs,activacion):
    # Generar los datos
    ...
    # Entrenar el modelo
    pesos,precision = train(x, t, xtest, ttest, pesos, learning_rate, epochs,activacion)
...
LEARNING_RATE = 0.1
EPOCHS = 10000
iniciar( NEURONAS_ENTRADA, NEURONAS_CAPA_OCULTA, 1, LEARNING_RATE, EPOCHS,"relu")